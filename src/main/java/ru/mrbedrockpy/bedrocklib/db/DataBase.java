...
@Getter
public class DataBase<P extends BedrockPlugin> extends ListManager<P, DataTable<? extends ManagerItem<?>, ?>, Class<?>> {
    ...
    public <T extends ManagerItem<ID>, ID> DataTable<T, ID> createTableIfNotExists(Class<T> dataType) {
        DataTable<T, ID> table = getTable(dataType);
    ...
    public <T extends ManagerItem<ID>, ID> DataTable<T, ID> getTable(Class<T> clazz) {
        for (DataTable<?, ?> table: tables) {
            if (table.getDataType().equals(clazz)) return (DataTable<T, ID>) table;
        }
        return null;
    }

    public List<DataTable<?, ?>> load() {
        tables.clear();
    ...
    public void save() {
        try {
            ...
            for (DataTable<?, ?> dataTable: tables) stringTables.add(dataTable.getDataType().getName() + "\n" + String.join("\n", dataTable.serialize()));
